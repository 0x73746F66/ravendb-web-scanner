...
def main(config_file):
  ...
  log = logging.getLogger()
  tld = get_from_model('tld', 'schema')
  if tld:
    sql(tld)
  domain = get_from_model('domain', 'schema')
  if domain:
    sql(domain)
  nameservers = get_from_model('nameservers', 'schema')
  if nameservers:
    sql(nameservers)
  link_domain_ns = get_from_model('link_domain_ns', 'schema')
  if link_domain_ns:
    sql(link_domain_ns)
  scan_log = get_from_model('scan_log', 'schema')
  if scan_log:
    sql(scan_log)

  setup_sql = None
  setup_file = path.join(path.realpath(getcwd()), 'sql', 'setup.sql')
  with open(setup_file, 'r') as f:
    setup_sql = load(f.read())
  if setup_sql:
    sql(setup_sql)
